...
export default function GenericForm(props) {
    ...
    const handleSubmit = (e) => {
        ...
        for (const input of inputs) {
            if (input?.validation?.required && !formData[input.name]) {
                ...
                if (
                    formData[input.name].length < input.validation.length.min ||
                    formData[input.name].length > input.validation.length.max
                ) {
                    if (message) {
                        errors[input.name] = input.validation.required.message
                    } else if (min && max) {
            ...
            else if (input?.validation?.pattern && !input.validation?.pattern.pattern.test(formData[input.name])) {
